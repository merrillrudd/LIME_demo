manipulate_snap <- function(data, sep_index){
...
obs_day <- obs_month <- obs_num <- rep(NA, length(years))
for(i in 1:length(years)){
	subyr <- data[which(data$Year==years[i]),]

	obs_day[i] <- length(unique(subyr$Date))
	obs_month[i] <- length(unique(subyr$Month))
	obs_num[i] <- nrow(subyr)
}
...
names(cpue_g) <- years
	...
	Outs$obs_day <- obs_day
	Outs$obs_month <- obs_month
	Outs$obs_num <- obs_num
	Outs$years <- years
	if(sep_index==TRUE){
